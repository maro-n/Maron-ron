	...
	// ゲーム本編
	{
		...
		while (1) {
			...
			// 左側のカードの処理
			{
				...
				bool on_card = false;
				if (mouse_pos.x() > x) {
					if (mouse_pos.x() < (x + left_card.texture_width)) {
						if (mouse_pos.y() > y) {
							if (mouse_pos.y() < (y + left_card.texture_height)) {
								on_card = true;
							}
						}
					}
				...
				Color card_color(1, 1, 1);
				if (on_card) {
					card_color = Color(1, 0.5, 0.5);
				}
			...
			// 右側のカードの処理
			{
				...
				bool on_card = false;
				if (mouse_pos.x() > x) {
					if (mouse_pos.x() < (x + right_card.texture_width)) {
						if (mouse_pos.y() > y) {
							if (mouse_pos.y() < (y + right_card.texture_height)) {
								on_card = true;
							}
						}
					}
